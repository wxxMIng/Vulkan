...
struct lightBuf1			Light1;	
struct lightBuf2			Light2;	// cpu struct to hold light information
...
InitGraphics( )
{
	...
	Init05UniformBuffer( 2 * sizeof(Light1),      &MyLightUniformBuffer );
	...
	Init07TextureBufferAndFillFromBmpFile("egpy1.bmp", &MyPuppyTexture);
...
layout( std140, set = 1, binding = 0 ) uniform lightBVuf1
{
	vec4 uLightPos1;
} Light1;

layout(std140, set = 1, binding = 0) uniform lightBVuf2
{
	vec4 uLightPos2;
} Light2;
...
Init13DescriptorSets( )
{
	...
	VkDescriptorBufferInfo				vdbi1;
		...
		vdbi1.range = sizeof(Light1, Light2);
...
Reset( )
{
	...
	Light2.uKd = 0.6f;
	Light2.uKs = 0.5f;
	Light2.uShininess = 300.f;
	Light2.uEyePos = glm::vec4(eye, 1.);
	Light2.uLightPos2 = glm::vec4( -100., -50., 200., 1.);
	Light2.uLightSpecularColor2 = glm::vec4(0., 1., 1., 1.);
